...
Стек (stack) является структурой данных, поддерживающей две операции: добавление элемента в вершину стека push (value) и удаление элемента из вершины стека pop (value). Дисциплина работы стека обозначается LIFO, последним пришел — первым ушел (Last In First Out). Стек можно реализовать при помощи массива.
   Как мы видим, заполняя стек методом push, мы помещаем каждый новый элемент в конец массива (Вершину стека). Указателем на вершину стека служит поле last в структуре стека. Снимая элемент с вершины стека методом pop в нашей реализации, мы возвращаем значение элемента, а затем удаляем этот элемент (уменьшая указатель last на единицу). 
Сложность каждой описанной операции О(1). Таким образом, обращаясь к вершине стека после его заполнения, мы разворачиваем последовательность элементов.

```c++
1:	#include <iostream> 
2:	#include <new.h> 
3:	using namespace std; 
4:	const int MAX_SIZE=10; 	//Количество элементов в массиве (Стеке) 
5:	
6:	struct my_stack //Описываем структуру стек 
7:	{ 
8:			int data[MAX_SIZE];	//Массив с количеством элементов MAX_SIZE 
9:			int last; //Указатель на вершину стека 
10:	}; 
11:	
12:	void push(my_stack &s, int x)	//Процедура добавления элемента 
13:	{ 
14:			if (s.last==MAX_SIZE)
15:			{   //Сообщение о переполнении стека 
...
20:	} 
21:	
22:	int pop(my_stack&s) //Получение элемента с вершины стека 	
23:	{ 
24:		    return s.data[--s.last];//Возвращаем элемент с вершины стека 
25:	} 
26:	
27:	int main() 
28:	{ 
...
32:			push(a,6); 
33:			push(a,2); 
34:			cout<<pop(a)<<" "; //Программа выведет 2 
35:			cout<<pop(a)<<" ";//Программа выведет 6 
36:			cout<<pop(a)<<" ";//Программа выведет 3 
37:			return 0; 
38:	}
...
<details> 
  <summary> 
    <strong>[Методы стека]</strong>
  </summary>
	
Стек можно применять для разворота последовательности элементов. Приведенную программу логично дополнить проверкой стека на наличие в нем элементов – методом empty(). Функция empty() вернет true, если стек пуст и false в противном случае. 
	
```c++
1:	bool empty(my_stack &s) 
2:	{ 
3:	     return (s.last==0); 
4:	} 
...
<details> 
  <summary> 
    <strong>[Простая реализация]</strong>
  </summary>
...
* **s[1…n]** — массив, с помощью которого реализуется стек, способный вместить не более **n** элементов,
* **s.top** — индекс последнего помещенного в стек элемента.

Стек состоит из элементов **s[1…s.top]**, где **s[1]** — элемент на дне стека, а **s[s.top]** — элемент на его вершине. Если **s.top=0**, то стек не содержит ни одного элемента и является пустым (англ. empty). Протестировать стек на наличие в нем элементов можно с помощью операции — запроса **stackEmpty**. Если элемент снимается с пустого стека, говорят, что он опустошается (англ. underflow), что обычно приводит к ошибке. Если значение **s.top** больше **n**, то стек переполняется (англ. overflow). (В представленном ниже псевдокоде возможное переполнение во внимание не принимается.) 
...
T pop():
  if empty()
    ...
    return s[s.top + 1]
```
Как видно из псевдокода выше, все операции со стеком выполняются за **O(1)**. 
